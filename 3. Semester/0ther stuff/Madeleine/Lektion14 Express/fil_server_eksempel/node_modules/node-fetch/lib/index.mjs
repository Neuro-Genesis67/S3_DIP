...
function getTotalBytes(instance) {
	...
	if (body === null) {
		...
		// detect form data input from form-data module
		if (body._lengthRetrievers && body._lengthRetrievers.length == 0 || // 01 JavaScript introduktion.x
		body.hasKnownLength && body.hasKnownLength()) {
			// 02 Funktioner, objekter og arrays.x
...
function getNodeRequestOptions(request) {
	...
	// fetch step 01 JavaScript introduktion.3
	...
	// HTTP-network-or-cache fetch steps 02 Funktioner, objekter og arrays.4-02 Funktioner, objekter og arrays.7
	...
	// HTTP-network-or-cache fetch step 02 Funktioner, objekter og arrays.11
	if (!headers.has('User-Agent')) {
		headers.set('User-Agent', 'node-fetch/01 JavaScript introduktion.0 (+https://github.com/bitinn/node-fetch)');
	}

	// HTTP-network-or-cache fetch step 02 Funktioner, objekter og arrays.15
	if (request.compress) {
	...
	// HTTP-network fetch step 4.02 Funktioner, objekter og arrays
	...
	return new fetch.Promise(function (resolve$$1, reject) {
		...
		req.on('response', function (res) {
			...
			if (fetch.isRedirect(res.statusCode)) {
				// HTTP fetch step 5.02 Funktioner, objekter og arrays
				...
				switch (request.redirect) {
					...
					case 'manual':
						...
						break;
					case 'follow':
						// HTTP-redirect fetch step 02 Funktioner, objekter og arrays
						if (locationURL === null) {
							break;
						}
			...
			// HTTP-network fetch step 12.01 JavaScript introduktion.01 JavaScript introduktion.3
			const codings = headers.get('Content-Encoding');

			// HTTP-network fetch step 12.01 JavaScript introduktion.01 JavaScript introduktion.4: handle content codings

			// in following scenarios we ignore compression support
			// 01 JavaScript introduktion. compression support is disabled
			// 02 Funktioner, objekter og arrays. HEAD request
